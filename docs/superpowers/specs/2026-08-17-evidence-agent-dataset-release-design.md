...
- 每个完整分卷固定为 100 MiB，即 104,857,600 字节。
- 最后一个分卷保存剩余字节，可以小于固定大小。
- 分卷不压缩，内容是源文件中对应区间的原始字节。
- 生成 638 个分卷，连同 2 个完整 Parquet 共 640 个 Release 附件，
  低于单个 Release 的 1,000 附件上限。
- 分卷名称包含源文件名、从 1 开始的序号和总卷数，例如：
  `repository_runtime.sqlite3.part-00001-of-00638`。
...
- SQLite 恰好生成 638 个分卷，两个 Parquet 各对应 1 个完整附件。
