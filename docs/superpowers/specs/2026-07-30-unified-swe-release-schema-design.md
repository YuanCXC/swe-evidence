...
当前机器有 12 个逻辑处理器，单 worker 实测平均仅使用约 2 个核，磁盘时间约 2%，因此本次断点构建采用 3 个 worker。切换时保留全部已提交状态，只处理 `status='pending'` 的文件版本，不重跑已完成行。恢复后分别在 2、5、10 分钟检查 SQLite 增量、进程树、工作集、可用内存和完整错误日志，并与切换前约 531 个文件版本/分钟的真实基线比较。

3-worker 运行必须同时满足以下门禁：

- 不出现可见终端窗口；
- 文件版本计数持续增长且 10 分钟吞吐至少比基线提高 30%；
- 构建进程树总工作集不超过 16 GiB，系统可用物理内存不低于 8 GiB；
- SQLite 无锁冲突、损坏或未提交状态丢失；
- 日志无新的构建错误，只有来自被分析源码的非阻断语法警告可以继续。

任一门禁失败时，先停止精确匹配当前构建命令的进程树，再从同一 SQLite 断点以 2 个无窗口 worker 恢复；若 2-worker 仍不满足资源或正确性门禁，则退回 1 个 worker。回退不得删除、重建或压缩状态库，也不得修改已经物化的数据语义。
