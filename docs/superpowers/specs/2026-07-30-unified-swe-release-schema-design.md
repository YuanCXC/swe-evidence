...
当前机器有 12 个逻辑处理器，单 worker 实测平均仅使用约 2 个核，磁盘时间约 2%。3-worker 运行的 10 分钟实测吞吐为约 2,314 个文件版本/分钟、总工作集约 2.81 GiB，已经通过正确性和资源门禁；在此基础上允许试运行 6 个 worker。切换时保留全部已提交状态，只处理 `status='pending'` 的文件版本，不重跑已完成行。恢复后分别在 2、5、10 分钟检查 SQLite 增量、进程树、工作集、可用内存和完整错误日志。

6-worker 试运行必须同时满足以下门禁：

- 不出现可见终端窗口；
- 文件版本计数持续增长且 10 分钟吞吐至少比 3-worker 基线提高 20%，即不低于约 2,780 个文件版本/分钟；Git blob 检查和下载等待时间单独记录，不把纯网络等待误判为物化并行失效；
