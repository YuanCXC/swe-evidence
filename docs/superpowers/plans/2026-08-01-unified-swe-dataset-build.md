...
- [x] 先加入临时 bare Git 仓库测试，覆盖 `repo + path + blob_oid` 去重、snapshot-path 唯一成员关系、正文哈希、二进制/vendor/generated 控制、文件级 unit 不可评分和超长单元继续分块。
- [x] 运行自测并确认新增测试失败。
- [x] 实现仓库缓存定位、commit/tree/blob 校验、流式 Git inventory、Python AST 与确定性窗口降级、import 声明提取、冻结 Tokenizer 计数和 SQLite 断点。
- [x] 在真实缓存上运行 `--through-phase corpus --audit-only`，验证 18,527 个唯一 snapshot 可解析、引用与正文哈希一致，缺失快照为 0。

## 任务 4：确定性监督、义务与 Witness Group

- [x] 先加入合成 patch、ContextBench Gold 和 SWE-Explore 区域 fixture，覆盖旧侧 patch 映射、包含区间合并、AND/OR witness、mandatory 条件和 663 个删除任务冻结。
- [x] 运行自测并确认新增测试失败。
- [x] 实现 patch 旧侧解析、overlay 映射、证据对齐、义务、witness 和 label provenance；SWE-Explore 的 `read_step_info` 保留在 SQLite 来源记录中供任务 5 构造状态，因其全部属于 benchmark，正式任务记录的 `trajectories` 按 Schema 保持为空。
- [x] 在真实数据运行 `--through-phase supervision --audit-only`，验证最终任务数量、删除原因分布和 Evidence Unit 引用完整率。
...
- [x] 加入本地假 HTTP 教师响应测试，覆盖 JSON Schema、引用白名单、技术重试、异步并发、结果顺序和教师状态隔离。
- [x] 按 TDD 重现并修复旧教师包的关系幻觉：排除重叠、包含和相同正文 pair，只保留双离线标签 pair 或 patch anchor 与 Issue 相关近邻。
- [x] 冻结 `unified-swe-teacher-v4`：教师只输出 obligation 和最小 witness group，`relations=[]`；程序按双离线来源、mandatory 和置信度下界推导高精度关系。
- [x] 实现 DeepSeek/OpenAI-compatible 教师客户端；冻结 `deepseek-v4-flash` 非思考模式、JSON Output 和 500 并发异步连接池，密钥只从环境变量或被 Git 忽略的 `.env` 读取，原始响应只存 SQLite。
- [x] 完成 30 个 v4 真实 pilot：12 个 `teacher_verified`、18 个 `teacher_unknown`、0 个规则拒绝，30 个响应均未输出关系对象；pilot 状态不计入正式教师包。
- [x] 冻结有效标签配额完成条件：拒绝/unknown 关闭 loss mask，并从同 split 未用 task/pair 补入替代候选；技术失败只重试同一包。只统计当前 Prompt 版本，保留 v2/v3 与 pilot 原始响应供审计。
- [x] 正式 v4 共调用 4,730 个包且无 pending/technical failure；0.8 强监督门禁后原始 verified 为 train 1,472、validation 401，已确定性冻结恰好 1,400/400 个有效标签，73 个超额 verified 作为关闭 loss mask 的备用。Manifest 写出阶段必须记录请求、verified/unknown/rejected、入选与备用数量，benchmark 不接收 teacher-only Gold。
...
- [ ] 配置教师环境变量后运行 `python scripts/build_unified_dataset.py --format parquet --release`；只有 18,347/223/2,294 个任务及五个文件全部通过硬门禁才发布。
