...
# -*- coding: utf-8 -*-
"""
audit_integrated_training_dataset_v1_1.py
...
建议放：scripts/audit_integrated_training_dataset_v1_1.py
"""
...
def state_action_errors(task_id: str, supervision: Mapping[str, Any]) -> list[str]:
    ...
    for state in states:
        ...
        if stype == "complete" and stop_label != "positive":
            errors.append(f"COMPLETE_STOP_NOT_POSITIVE:{task_id}:{state_id}:{stop_label}")
        if stype != "complete" and stop_label == "positive":
            errors.append(f"NONCOMPLETE_STOP_POSITIVE:{task_id}:{state_id}")
        ...
        for action in actions:
            ...
            if label == "positive":
                if action.get("scoreable") is not True:
                    errors.append(f"POSITIVE_UNSCOREABLE:{task_id}:{aid}")
                if action.get("action_loss_mask") is not True:
...
def self_test() -> int:
    supervision = {
        "obligations": [{"witness_groups": [{"evidence_ids": ["a", "b"]}]}],
        "hard_negative_evidence_ids": [],
        "policy_states": [
            {
                "state_id": "i", "state_type": "initial", "evidence_ids": [], "stop_label": "negative",
                "candidate_actions": [
            ...
            {
                "state_id": "c", "state_type": "complete", "evidence_ids": ["a", "b"], "stop_label": "positive",
                "candidate_actions": [
    ...
    for split in SPLITS:
        ...
        while True:
            ...
            for base, der in zip(brows, drows):
                ...
                # 所有 task 的 Policy structural integrity 都要检查。
