...
-------------------------------
python scripts/export_final_experiment_bundle_v1_0.py `
...
def export_policy_evidence(
    *,
    ...
    try:
        ecols = table_columns(conn, "evidence_units")
        if not ecols:
            raise RuntimeError("Working DB evidence_units has no columns")
        enames = [c["name"] for c in ecols]
        if "evidence_id" not in enames:
            raise RuntimeError("evidence_units is missing evidence_id")
        if "file_version_id" not in enames:
            raise RuntimeError("evidence_units is missing file_version_id")

        fcols = table_columns(conn, "file_versions")
        ...
        for col in ecols:
            name = col["name"]
            select_exprs.append(f"e.{qident(name)} AS {qident(name)}")
            fields.append(pa.field(name, sqlite_decl_to_arrow(col["type"])))
            out_names.add(name)

        # Add a few file-level fields only when Evidence Unit itself does not already carry them.
        ...
        ftype = {c["name"]: c["type"] for c in fcols}
        for name in preferred_file_fields:
            if name in fnames and name not in out_names:
                select_exprs.append(f"fv.{qident(name)} AS {qident(name)}")
                fields.append(pa.field(name, sqlite_decl_to_arrow(ftype.get(name))))
                out_names.add(name)

        # At minimum the renderer needs stable identity plus source text. We accept common text names.
        if not any(x in out_names for x in ("content", "text", "source", "code")):
            raise RuntimeError(
                "Cannot export policy evidence: evidence_units has no content/text/source/code column. "
                f"Columns={sorted(out_names)}"
            )
        if "path" not in out_names:
            raise RuntimeError(
                "Cannot export policy evidence: neither evidence_units nor file_versions exposes path"
            )
        ...
        try:
            while True:
                ...
                for idx, field in enumerate(schema):
                    vals = [_coerce_for_arrow(row[idx], field.type) for row in rows]
                    arrays.append(pa.array(vals, type=field.type))
                table = pa.Table.from_arrays(arrays, schema=schema)
                writer.write_table(table)
                rows_written += len(rows)
                if rows_written % 200_000 < len(rows):
                    print(
...
def build_generic_runtime_fts(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    if not id_col or not path_col or not content_col:
        raise RuntimeError(
            "Cannot build generic runtime FTS; required file_versions columns not found. "
    ...
    conn.execute(
        ...
        f"SELECT CAST({qident(id_col)} AS TEXT), {qident(path_col)}, {qident(content_col)} "
        "FROM file_versions"
    )
    return {
...
def export_runtime_db(
    *,
    ...
    try:
        ...
        src_uri = build_db.resolve().as_uri() + "?mode=ro"
        fts_uri = fts_db.resolve().as_uri() + "?mode=ro"
        conn.execute("ATTACH DATABASE ? AS src", (src_uri,))
        conn.execute("ATTACH DATABASE ? AS srcfts", (fts_uri,))
        ...
        try:
            print("[bundle] runtime DB: merging V2.10 FTS5 sidecar ...", file=sys.stderr, flush=True)
            fts_report = copy_fts_sidecar_exact(conn, fts_schema="srcfts")
            conn.commit()
        except Exception as exc:
            conn.rollback()
            if not allow_generic_fts_fallback:
                raise RuntimeError(
                    "Failed to merge the historical V2.10 FTS sidecar exactly. "
                    "If you accept a canonical generic runtime FTS schema, re-run with "
                    "--allow-generic-fts-fallback. Original error: " + repr(exc)
                ) from exc
            print(
                f"[bundle] exact FTS merge failed; building generic fallback: {exc!r}",
                file=sys.stderr,
                flush=True,
            )
        ...
        # Runtime integrity check before detaching/closing.
        integrity = conn.execute("PRAGMA integrity_check").fetchone()[0]
        if str(integrity).lower() != "ok":
            raise RuntimeError(f"repository_runtime.sqlite3 integrity_check failed: {integrity}")

        conn.execute("DETACH DATABASE srcfts")
        ...
        conn.close()
        # WAL/SHM should not survive, but make the four-file contract explicit.
