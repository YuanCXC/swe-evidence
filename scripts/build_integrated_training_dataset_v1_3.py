...
# -*- coding: utf-8 -*-
"""
build_integrated_training_dataset_v1_3.py
...
OUTPUT_DATASET_VERSION = "2.10.0+strong_teacher_mechanical_v1.3"
...
def positive_scoreability_violations(states: Sequence[Mapping[str, Any]]) -> list[str]:
    """返回 positive 但不可评分/被 mask 的 action_id。V2.10 release gate 要求这里为空。"""
    bad: list[str] = []
    for state in states:
        for action in (state.get("candidate_actions") or []):
            if not isinstance(action, Mapping):
                continue
            if str(action.get("action_label") or "") != "positive":
                continue
            if action.get("scoreable") is not True or action.get("action_loss_mask") is not True:
                bad.append(str(action.get("action_id") or ""))
...
class PolicyRuntime:
    ...
    def rollback_rebuild_counters(self, states: Sequence[Mapping[str, Any]]) -> None:
        """当新 Teacher supervision 违反 scoreability gate 时撤销本次 rebuild 统计。"""
        self.rebuilt_tasks -= 1
        self.rebuilt_states -= len(states)
        self.rebuilt_actions -= sum(len(s.get("candidate_actions") or []) for s in states)
        if self.rebuilt_tasks < 0 or self.rebuilt_states < 0 or self.rebuilt_actions < 0:
            raise RuntimeError("PolicyRuntime rebuild counters underflow")
...
def extension_fields(pa):
    return [
        ...
        pa.field("experiment_exclusion_source", pa.string()),
        pa.field("experiment_exclusion_details_json", pa.large_string()),
    ]
...
def build_output_row(
    *,
    ...
    if obligations_changed:
        ...
        rebuilt_states = policy_runtime.rebuild(
            base_row=row, supervision=effective_supervision
        )
        effective_supervision["policy_states"] = rebuilt_states
        policy_rebuilt = True

        bad_positive_actions = positive_scoreability_violations(rebuilt_states)
        if bad_positive_actions:
            # 不回退旧 V2.10 supervision：旧监督未必更正确。
            # 保留 Teacher-derived supervision/policy 仅用于 provenance/audit，
            # 并把整个 task 从训练、validation/model-selection、benchmark evaluation
            # 及聚合实验指标中排除。
            experiment_eligible = False
            experiment_exclusion_reason = "POLICY_POSITIVE_UNSCOREABLE"
            experiment_exclusion_source = "integrated_builder_v1.3"
            experiment_exclusion_details_json = stable_json({
                "bad_positive_action_ids": bad_positive_actions,
                "policy": (
                    "retain Teacher-derived supervision for provenance/audit only; "
                    "exclude from all experiment consumption; no fallback to older supervision"
                ),
            })
            policy_exclusion_reason = "POLICY_POSITIVE_UNSCOREABLE_EXPERIMENT_EXCLUDED"
            blockers = sorted(set([
                *blockers,
                "policy:positive_unscoreable_experiment_excluded",
                *(f"policy:unscoreable_positive:{aid}" for aid in bad_positive_actions),
            ]))
    ...
    row.update(
        {
            ...
            "strong_teacher_risk_score": int(teacher_row.get("risk_score") or 0),
            "strong_teacher_risk_flags_json": str(teacher_row.get("risk_flags_json") or "[]"),
            "strong_teacher_exclusion_reason": policy_exclusion_reason,
            ...
            "experiment_exclusion_reason": experiment_exclusion_reason,
            "experiment_exclusion_source": experiment_exclusion_source,
            "experiment_exclusion_details_json": experiment_exclusion_details_json,
        }
    )
...
def write_split(
    *,
    ...
    try:
        for batch in parquet.iter_batches(batch_size=batch_size):
            ...
            for base_row in rows:
                ...
                experiment_excluded_count += int(out_row.get("experiment_eligible") is False)
    ...
    return {
        ...
        "experiment_excluded_tasks": experiment_excluded_count,
...
def self_test() -> int:
    ...
    assert positive_scoreability_violations([
        {"candidate_actions": [
            {"action_id": "ok", "action_label": "positive", "scoreable": True, "action_loss_mask": True},
            {"action_id": "bad", "action_label": "positive", "scoreable": False, "action_loss_mask": False},
        ]}
    ]) == ["bad"]
    print("SELF_TEST_OK")
    return 0
            ...
            "integration": {
                ...
                "experiment_eligibility": {
                    ...
                    "excluded_task_count": sum(
                        int(split_reports[s].get("experiment_excluded_tasks") or 0)
                        for s in SPLITS
                    ),
                    "excluded_by_split": {
                        s: int(split_reports[s].get("experiment_excluded_tasks") or 0)
                        for s in SPLITS
                    },
                    "reason": "POLICY_POSITIVE_UNSCOREABLE",
                    "rule": (
                        "All training, validation/model-selection, benchmark evaluation, and aggregate metrics "
                        "must filter experiment_eligible == true. Excluded rows remain for provenance/audit."
                    ),
                    "old_supervision_fallback": False,
                    "physical_row_deletion": False,
                },
                "positive_scoreability_policy": (
                    "keep Teacher-derived supervision for provenance/audit; mark whole task experiment_eligible=false; "
                    "no truncation; no fallback to older supervision; no semantic guessing"
                ),
