...
def export_runtime_db(
    *,
    ...
    # Windows note:
    # Do NOT combine locking_mode=EXCLUSIVE with read-only URI ATTACH here.  On Windows,
    # SQLite may try to establish locking/shared-memory state for the attached database and
    # fail with SQLITE_IOERR / "disk I/O error" even though the source DB itself is healthy.
    #
    # We attach the source databases by their absolute filesystem paths and keep every SQL
    # statement against src/srcfts strictly read-only.  The destination database is the only
    # database that receives CREATE/INSERT/INDEX statements.
    ...
    try:
        ...
        fts_path = str(fts_db.resolve())
        try:
            conn.execute("ATTACH DATABASE ? AS src", (src_path,))
            src_attached = True
            # Force a real read immediately so attach/locking problems fail before we create
            # any large destination tables.
        ...
        try:
            conn.execute("ATTACH DATABASE ? AS srcfts", (fts_path,))
            fts_attached = True
            conn.execute("SELECT COUNT(*) FROM srcfts.sqlite_master").fetchone()
        except sqlite3.Error as exc:
            raise RuntimeError(
                "Cannot attach the V2.10 FTS sidecar for read-only export. "
                f"path={fts_path!r}, sqlite_error={exc!r}. "
                "Close programs that may hold the DB exclusively and retry."
            ) from exc
        ...
        # Detach source DBs before optimizing/finalizing the destination.
        if fts_attached:
        ...
        # Best-effort detach on exceptional paths.  Roll back first because SQLite refuses
        # DETACH while a transaction touching the attached DB is active.
        try:
            conn.rollback()
        except Exception:
            pass
