...
14. 429 与 503/504 使用不同退避策略；503/504 视为上游 backend busy。
15. 多个 key 在短窗口内同时出现 503/504 时触发 provider-level global cooldown。
16. 所有 retry / cooldown 等待均发生在 key release 之后，不占用 API key 租约。
...
class ProviderCooldown:
    """
    Provider-level backend busy 冷却器。

    目的：
    当不同 API key 在很短时间内同时得到 503/504 时，问题通常来自共享上游
    inference backend / gateway，而不是单个 key。此时继续让其他 worker 立刻发请求
    只会制造更多 503/504。

    规则：
    - register_busy() 记录 503/504 时间；
    - 在 window_seconds 内达到 threshold 次，触发 cooldown_seconds 全局冷却；
    - wait_if_active() 在发起新请求前阻塞；
    - 冷却不禁用任何 key；
    - 线程安全；
    - 冷却期间不应持有 KeyPool lease。
    """
    ...
    def __init__(
        ...
        if threshold < 1:
            raise ValueError("busy cooldown threshold 必须 >= 1")
        if window_seconds <= 0:
            raise ValueError("busy cooldown window 必须 > 0")
        if cooldown_seconds <= 0:
            raise ValueError("busy cooldown seconds 必须 > 0")
    ...
    def register_busy(self) -> tuple[bool, float]:
        """
        记录一次 503/504。

        返回:
            (triggered_new_cooldown, remaining_seconds)
        """
        now = time.monotonic()
        with self._cond:
            ...
            if len(self._events) >= self.threshold:
                new_until = now + self.cooldown_seconds
                if new_until > self._cooldown_until:
                    self._cooldown_until = new_until
                    triggered = True

                # 一次 burst 只需要触发一个 cooldown。
                # 清空窗口，避免同一批 503 每个都把 cooldown 再向后推 30 秒。
                self._events.clear()
    ...
    def wait_if_active(self) -> None:
        """
        若 provider 正在全局冷却，则等待到 cooldown 结束。
        调用者必须确保当前没有持有 API key lease。
        """
        with self._cond:
            while True:
...
def get_http_status(exc: BaseException) -> Optional[int]:
    """
    从 OpenAI SDK APIStatusError / 子类提取 HTTP status。
    某些兼容服务会返回 HTML 504，但 SDK 仍会在 status_code 中保留 504。
    """
...
def classify_retry_error(exc: BaseException) -> str:
    """
    返回机械 retry 分类，不做业务语义判断。

    kinds:
      quota         明确额度/余额耗尽，禁用当前 key
      rate_limit    429
      backend_busy  503 / 504
      server_error  500 / 502
      transient     408 / 409 / connection / timeout
      validation    本地 JSON/schema/task_id/Candidate binding 失败
      fatal         其它错误，不自动重试
    """
...
def retry_delay_seconds(kind: str, attempt: int) -> float:
    """
    attempt 是刚失败的 attempt（从 1 开始）。

    目标：
    - 429:       5s, 15s, 30s, 60s...
    - 503/504:  10s, 30s, 60s, 120s...
    - 500/502:   5s, 15s, 30s, 60s...
    - network:   3s, 10s, 20s, 40s...
    - validation:1s, 2s, 4s, 8s...

    加少量 jitter，避免多个 worker 同时苏醒形成 thundering herd。
    """
    schedules = {
        "quota": [0, 0, 0, 0],
        "rate_limit": [5, 15, 30, 60],
        "backend_busy": [10, 30, 60, 120],
        "server_error": [5, 15, 30, 60],
        "transient": [3, 10, 20, 40],
        "validation": [1, 2, 4, 8],
    }
    seq = schedules.get(kind, [5, 15, 30, 60])
    idx = max(0, attempt - 1)
    base = seq[min(idx, len(seq) - 1)]
    return float(base) + random.uniform(0.0, 1.5)
...
def process_one_task(
    ...
    for attempt in range(1, args.max_retries + 1):
        # -------------------------------------------------------------
        # 先等 provider cooldown，再拿 key。
        # 如果 worker 已经在 KeyPool.acquire_key() 中等待，而期间另一个请求
        # 刚触发了全局 cooldown，则拿到 key 后再次检查；若 cooldown 已激活，
        # 立即释放 key 并等待，绝不拿着 key 睡觉。
        # -------------------------------------------------------------
        while True:
        ...
        success_json: Optional[str] = None
        ...
        try:
            ...
            atomic_write_text(output_md, success_json)
            ...
            retry_kind = classify_retry_error(exc)
            quota_error = retry_kind == "quota"
            ...
            if retry_kind == "backend_busy":
                cooldown_triggered, cooldown_remaining = cooldown.register_busy()
                if cooldown_triggered:
                    safe_tqdm_write(
                        f"[GLOBAL COOLDOWN] provider={provider.name} | "
                        f"BUSY | recent 503/504 threshold reached | "
            ...
            if should_retry:
                retry_delay = retry_delay_seconds(retry_kind, attempt)

        finally:
            # 核心不变量：
            # retry / global cooldown / sleep-between 全都不能占用 key lease。
            key_pool.release(key_slot)
        ...
        if should_retry:
            active_keys = key_pool.active_count()
            cooldown_left = cooldown.remaining_seconds()

            safe_tqdm_write(
                f"[RETRY {attempt}/{args.max_retries}] "
                f"{task_id} | provider={provider.name} | key#{key_slot} | "
                f"kind={retry_kind} | {type(last_error).__name__}: {last_error} | "
                f"active_keys={active_keys} | "
                f"retry_sleep={retry_delay:.1f}s | "
                f"global_busy={cooldown_left:.1f}s"
            )

            # 先执行本错误类型自己的短/长退避。
            # 下一轮开始时还会调用 cooldown.wait_if_active()；
            # 因此若全局 cooldown 比 retry_delay 更长，只会等待“剩余时间”，
            # 不会把 10s + 30s 简单叠成 40s。
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=int,
        default=5,
        help="每个 task 最大尝试次数，默认 5。",
    )
    p.add_argument(
        "--busy-cooldown-threshold",
        type=int,
        default=2,
        help="在时间窗口内出现多少次 503/504 后触发全局冷却；默认 2。",
    )
    p.add_argument(
        "--busy-cooldown-window",
        type=float,
        default=10.0,
        help="统计 503/504 burst 的时间窗口（秒）；默认 10。",
    )
    p.add_argument(
        "--busy-cooldown-seconds",
        type=float,
        default=30.0,
        help="触发 backend busy 后 provider 全局冷却秒数；默认 30。",
    )
    p.add_argument(
        "--timeout",
        type=float,
...
def main() -> int:
    ...
    if args.busy_cooldown_threshold < 1:
        raise SystemExit("--busy-cooldown-threshold 必须 >= 1")
    if args.busy_cooldown_window <= 0:
        raise SystemExit("--busy-cooldown-window 必须 > 0")
    if args.busy_cooldown_seconds <= 0:
        raise SystemExit("--busy-cooldown-seconds 必须 > 0")

    load_environment(args.env_file)
    provider = load_provider_config(args)
    key_pool = KeyPool(provider.api_keys)
    cooldown = ProviderCooldown(
        threshold=args.busy_cooldown_threshold,
        window_seconds=args.busy_cooldown_window,
        cooldown_seconds=args.busy_cooldown_seconds,
    )
    ...
    with ThreadPoolExecutor(
        ...
        while futures:
            ...
            progress.set_postfix(
                ...
                fail=stats.failed,
                active_keys=key_pool.active_count(),
                busy=f"{cooldown.remaining_seconds():.0f}s",
        ...
        + json.dumps(
            {
                ...
                "concurrency": concurrency,
                "busy_cooldown_remaining_seconds": round(
                    cooldown.remaining_seconds(), 3
                ),
                "scanned": stats.scanned,
