...
def make_progress(iterable, *, total=None, desc="", unit="it", enabled=True):
...
def load_model_and_tokenizer(args: argparse.Namespace) -> tuple[Any, Any, Path | str]:
    ...
    if args.resume_from is not None:
        source: Path | str = args.resume_from.resolve()
        revision = None
...
def main() -> int:
    ...
    try:
        ...
        if args.resume_from is not None:
            ...
            optimizer_state = resume / "optimizer.pt"
            scheduler_state = resume / "scheduler.pt"
            trainer_state = resume / "trainer_state.json"
            if optimizer_state.is_file():
                optimizer.load_state_dict(
                    torch.load(optimizer_state, map_location="cpu")
                )
            if scheduler_state.is_file():
                scheduler.load_state_dict(
                    torch.load(scheduler_state, map_location="cpu")
                )
            if trainer_state.is_file():
                saved = json.loads(trainer_state.read_text(encoding="utf-8"))
                start_epoch = int(saved.get("epoch", -1)) + 1
                global_state_step = int(saved.get("global_state_step", 0))
                global_optimizer_step = int(saved.get("global_optimizer_step", 0))
                best_mrr = float(saved.get("best_validation_mrr", -1.0))
                no_improve_epochs = int(saved.get("no_improve_epochs", 0))
                saved_loss_ema = saved.get("loss_ema")
