...
def validate_slot(
    ...
    if repo_need in {"not_needed", "not_applicable", "helpful"} and groups:
        raise ValueError(
    ...
    if repo_need == "required":
        ...
        if status in {"insufficient", "uncertain"} and groups:
            raise ValueError(
                f"{slot_name}: candidate_pool_status={status} 却存在 Witness"
            )
        ...
        try:
            ...
            for slot_name, warning in schema_warnings:
                ...
                if (
                    ...
                    issue(
                        issues, "HARD_ERROR", warning.split(":")[0],
                        r.split, r.filename, q.task_id or "",
                        f"{slot_name}: {warning}",
                    )
                else:
                    issue(
                        issues, "RISK_FLAG", warning.split(":")[0],
                        r.split, r.filename, q.task_id or "",
                        f"{slot_name}: {warning}",
                        1,
                    )
