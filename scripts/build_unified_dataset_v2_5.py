...
DATASET_NAME = "unified_swe_dataset_v2_5"
DATASET_VERSION = "2.5.0"
SCHEMA_VERSION = "1.0"
SCRIPT_VERSION = "0.2.5"
RELEASE_TAG = "v2_5"
MANIFEST_FILENAME = "manifest_v2_5.json"
...
V2_STAGING_ROOT = Path("data/unified_swe_dataset_v2_5.tmp")
V2_RELEASE_ROOT = Path("data/unified_swe_dataset_v2_5")

RELEASE_FILES = (
    "train_v2_5.parquet",
    "validation_v2_5.parquet",
    "benchmark_v2_5.parquet",
    "repository_corpus_v2_5.parquet",
    MANIFEST_FILENAME,
)
...
RETRIEVER_VERSION = "retriever-v2.5-inplace-rowid-fts-lazy-static-q-head-rrf"
POLICY_PHASE_VERSION = "2.5.0"
RRF_K = 64
CHANNEL_DEPTH = 64
FINAL_DEPTH = 64
...
# V2.5：只缓存派生的 policy records / 检索词项，不修改任何监督或语义标签。
# 2048 个 file_version 通常只占数百 MB；如机器内存紧张可调小。
POLICY_FILE_RECORD_CACHE_MAX = 2_048

# V2.5：FTS 直接限制到当前 snapshot 的 FTS rowid，避免先扫描 repo 全量命中再 Python 过滤。
FTS_ROWID_QUERY_CHUNK = 2_000
_POLICY_FTS_ROWID_BY_FILE_VERSION: dict[str, int] = {}
...
def policy_records_from_file_payload(
    ...
    """V2.5：把 scoreable unit 转成轻量策略记录。
...
def precompute_task_query_channel_rankings(
    ...
    """V2.5：BM25/path/symbol 是 q-only 通道，每个 task 只完整排序一次。
...
def _ensure_policy_fts_rowid_map(
    fts_connection: sqlite3.Connection,
) -> dict[str, int]:
    """一次性加载约 95 万个 file_version -> FTS rowid 映射。

    该映射只驻留在当前 Python 进程内，不写入 60GB working DB。
    """

    global _POLICY_FTS_ROWID_BY_FILE_VERSION
    if _POLICY_FTS_ROWID_BY_FILE_VERSION:
        return _POLICY_FTS_ROWID_BY_FILE_VERSION

    started = time.perf_counter()
    _POLICY_FTS_ROWID_BY_FILE_VERSION = {
        str(row["file_version_id"]): int(row["rowid"])
        for row in fts_connection.execute(
            "SELECT rowid, file_version_id FROM policy_file_fts_map"
        )
    }
    print(
        "retrieval-index-v2.5: loaded "
        f"{len(_POLICY_FTS_ROWID_BY_FILE_VERSION)} FTS rowid mappings "
...
def query_policy_file_fts(
    ...
    *,
    ...
    """V2.5：FTS MATCH 直接约束到当前 snapshot 的 rowid 集合。

    V2.4 的做法是：
        repo FTS 全局排序 -> Python membership 过滤
    当某 repo 有大量历史 file_version 时，会扫描很多与当前 snapshot 无关的命中。

    V2.5 先把 snapshot 的 file_version_id 映射成 FTS rowid，然后分块执行：
        MATCH query AND rowid IN (current snapshot rows)
    每块只保留 Top-cap，最后全局合并；全局 Top-cap 必然包含在各块 Top-cap 的并集中。
    """
    ...
    rowid_by_file = _ensure_policy_fts_rowid_map(fts_connection)
    snapshot_rowids = sorted(
        rowid_by_file[file_version_id]
        for file_version_id in membership_file_ids
        if file_version_id in rowid_by_file
    )
    if not snapshot_rowids:
        return []
    ...
    for offset in range(0, len(snapshot_rowids), FTS_ROWID_QUERY_CHUNK):
        chunk = snapshot_rowids[offset : offset + FTS_ROWID_QUERY_CHUNK]
        placeholders = ",".join("?" for _ in chunk)
        sql = (
            ...
            "AND policy_file_fts.rowid IN (" + placeholders + ") "
            "AND rank MATCH 'bm25(0.0, 1.5, 1.0)' "
            "ORDER BY rank ASC, policy_file_fts.rowid ASC "
            "LIMIT ?"
        )
        params: list[Any] = [match_query, *chunk, int(cap)]
        for row in fts_connection.execute(sql, params):
            file_version_id = str(row["file_version_id"])
            candidate = {
                "path": str(row["path"]),
                "file_version_id": file_version_id,
            ...
            previous = candidates.get(file_version_id)
            if previous is None or candidate["fts_score"] < previous["fts_score"]:
                candidates[file_version_id] = candidate

    return sorted(
        candidates.values(),
        key=lambda item: (float(item["fts_score"]), str(item["file_version_id"])),
    )[:cap]
...
def render_policy_model_inputs(
    *,
    ...
    """V2.5：批量渲染，并复用 body-fit 过程中已经得到的精确 token count。
...
def _load_policy_evidence_universe(
    ...
    *,
    ...
    """V2.5：path + snapshot-aware sidecar FTS5 两路文件召回，另行补齐离线 witness。"""
    ...
    if fts_connection is None:
        raise ValueError("V2.5 policy universe 需要已打开的 sidecar FTS connection。")
    membership_file_ids = {
    ...
    *,
    ...
    try:
            ...
            stable_json_dumps(
                {
                    ...
                    "fts_snapshot_constraint": "rowid_in_snapshot_chunks_v1",
        ...
        if not compatible_phase:
            ...
            print(
                f"V2.3 inplace invalidation: replacing old policy "
        ...
        for local_index, row in enumerate(rows_to_process, 1):
            task_id = str(row["task_id"])
            task_payload = json.loads(row["task_json"])
            # V2.5 原地模式把 V1 最终 supervision 当作冻结事实；
            ...
            # 原地 V2.5 只写 policy_states / candidate_actions。
    ...
    try:
        ...
        for phase_name in ("split", "snapshots", "corpus", "supervision", "teacher"):
            ...
            if row is None or row["completed_at"] is None:
                raise ValueError(f"原地 V2.5 需要已完成的 V1 {phase_name} checkpoint。")
...
def run_cli(
    ...
    *,
    ...
    if target_phase_index >= policy_phase_index_for_guard:
        ...
        print(
            "V2.5 inplace mode: reuse frozen V1 SQLite; only policy/downstream phases may change",
            file=sys.stderr,
            flush=True,
        )
        print(
            f"V2.5 working state: {inplace_report['state_path']}",
            file=sys.stderr,
            flush=True,
        )
    if (
...
class PolicyTests(unittest.TestCase):
    ...
    def test_manifest_records_four_data_files_without_self_hash(self) -> None:
        files = {
            ...
            for name, count in (
                ("train_v2_5.parquet", 18_347),
                ("validation_v2_5.parquet", 223),
                ("benchmark_v2_5.parquet", 2_294),
                ("repository_corpus_v2_5.parquet", 1_027_752),
            )
        }
        manifest = build_release_manifest(
            files,
    ...
    def test_staged_audit_recomputes_counts_sizes_and_hashes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in (
                "train_v2_5.jsonl",
                "validation_v2_5.jsonl",
                "benchmark_v2_5.jsonl",
                "repository_corpus_v2_5.jsonl",
            ):
                path = root / name
                path.write_text("{}\n", encoding="utf-8")
                reports[name] = {
